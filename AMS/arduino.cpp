...
int arduino::connect_arduino(QString port) {
...

{
    while (serial->bytesToWrite() > 0) {
        qDebug() << "Waiting for write job to end...";
...

{
    if (serial->isWritable()) {
       if (serial->write(d) == -1) // envoyer des donnés vers arduino
            qDebug() << "An error occured while sending serial";
